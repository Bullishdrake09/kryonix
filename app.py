...
# ── FIX: Use env var for secret key in production; never hardcode ──
...
# ── FIX: CSRF token stored in session; validated on all mutating requests ──
...
def validate_csrf(token):
    """Return True if the supplied token matches the session token."""
    return token and token == session.get('csrf_token')

# ── FIX: Validate file extensions properly ──
...
def get_user_by_username_or_email(login_field):
    ...
    for username, data in all_users.items():
        if username == login_field or data.get('email') == login_field:
...
def add_message_to_history(room, message_data):
    ...
    try:
        ...
        # ── FIX: Validate room format before splitting ──
        parts = room.split('-')
        if len(parts) != 2:
            app.logger.error(f"Invalid room name format: {room}")
            return False
...
# ── FIX: Helper to strip HTML tags for safe storage/comparison ──
...
def register():
    ...
    if request.method == 'POST':
        ...
        # ── FIX: Reject usernames with special chars that could break room names ──
        if not re.match(r'^[\w.-]+$', username):
            flash('Username may only contain letters, numbers, dots, dashes, and underscores.', 'error')
            return render_template('register.html', error='Invalid username characters.')
...
def history(room_name):
    try:
        ...
        limit = min(int(request.args.get('limit', MESSAGES_PER_PAGE)), 100)  # ── FIX: cap limit ──
        ...
        if target_username not in current_user_data.get('friends', []):
            ...
            if target_user_data and current_user.username in target_user_data.get('blocked', []):
                return jsonify({'error': f'You are blocked by {target_username}.'}), 403
...
def profile_picture(filename):
    ...
    if safe_filename != filename:
        abort(400)
    return send_from_directory(app.config['PROFILE_PICS_FOLDER'], safe_filename)
...
def upload_file():
    try:
        ...
        # ── FIX: Validate extension before saving ──
        if not allowed_file(file.filename, ALLOWED_UPLOAD_EXTENSIONS):
            return jsonify({'error': 'File type not allowed. Only images and videos are accepted.'}), 400
...
def upload_profile_picture():
    try:
        ...
        # ── FIX: Only allow images for profile pictures ──
        if not allowed_file(file.filename, ALLOWED_IMAGE_EXTENSIONS):
            return jsonify({'error': 'Invalid file type. Only images (PNG, JPG, JPEG, GIF, WebP) allowed.'}), 400
        ...
        # ── FIX: Handle case where user_data is None ──
        if not user_data:
            return jsonify({'error': 'User data not found'}), 404
...
def create_group():
    try:
        ...
        # ── FIX: at least 1 other member (was requiring 2, which is too strict) ──
        ...
        if save_group_chats(group_chats):
            # Real-time: notify all members (except creator who gets it via the API response)
            for member in member_usernames:
...
def update_group(group_id):
    try:
        ...
        if action == 'rename':
            ...
            group['name'] = new_name
            if save_group_chats(group_chats):
                # Real-time: tell all members about the rename
                for member in group.get('members', []):
                    if member in user_sids:
                        for sid in user_sids[member]:
                            socketio.emit('group_membership_update', {
            ...
            if member_to_kick in group['members']:
                ...
                if save_group_chats(group_chats):
                    # Real-time: tell the kicked user and all remaining members
                    if member_to_kick in user_sids:
                        for sid in user_sids[member_to_kick]:
                            socketio.emit('group_membership_update', {
                                'action': 'removed',
                                'group_id': group_id,
            ...
            if save_group_chats(group_chats):
                # Real-time: tell each new member they were added
                for new_member in new_members:
                    if new_member in user_sids:
                        for sid in user_sids[new_member]:
                            socketio.emit('group_membership_update', {
                                'action': 'added',
                                'group_id': group_id,
                                'group_name': group.get('name', '')
            ...
            if current_user.username in group['members']:
                ...
                if save_group_chats(group_chats):
                    # Real-time: confirm to the leaving user (they may have multiple tabs)
                    if current_user.username in user_sids:
                        for sid in user_sids[current_user.username]:
                            socketio.emit('group_membership_update', {
                                'action': 'removed',
                                'group_id': group_id,
                                'group_name': group.get('name', '')
...
def get_pending_requests_count():
    """Lightweight endpoint polled every 30s by the client for the friend-request badge."""
    try:
        user_data = load_user_data(current_user.username)
        count = len(user_data.get('requests', [])) if user_data else 0
        return jsonify({'count': count})
    except Exception as e:
        app.logger.error(f"Error getting pending requests count: {e}")
...
def get_contacts_order():
    try:
        ...
        for friend in friends:
            ...
            if messages:
                last_msg = messages[-1]
                last_msg_text = last_msg.get('msg', '')
                    ...
                    if action == 'send_request':
                        if target_username in user_data['friends'] or \
                           ...
                           username in target_user_data.get('requests', []):
                            ...
                            message = f"You have blocked '{target_username}'. Unblock them first."
                            message_type = "error"
                        elif username in target_user_data.get('blocked', []):
                            message = f"You are blocked by '{target_username}'."
                            message_type = "error"
                        else:
                            ...
                            if save_user_data(target_user_data):
                                ...
                                # Real-time: notify the recipient immediately
                                pending_count = len(target_user_data.get('requests', []))
                        ...
                        if target_username in user_data['requests']:
                            ...
                            if save_user_data(user_data) and save_user_data(target_user_data):
                                ...
                                message_type = "success"
                                # Real-time: push the new friend into each other's sidebars
                                all_users_rt = load_all_users_data()
                                my_pic     = all_users_rt.get(username, {}).get('profile_picture')
                                their_pic  = all_users_rt.get(target_username, {}).get('profile_picture')
                                remaining  = len(user_data.get('requests', []))
                                # Notify the person who accepted: refresh their badge + add friend
                                if username in user_sids:
                                    for sid in user_sids[username]:
                                        socketio.emit('friend_request_accepted', {
                                            'username': target_username,
                                            'profile_picture': their_pic,
                                            'pending_count': remaining
                                        }, room=sid)
                                # Notify the original requester: they are now friends
                                if target_username in user_sids:
                                    for sid in user_sids[target_username]:
                                        socketio.emit('friend_request_accepted', {
                                            'username': username,
                                            'profile_picture': my_pic,
                                            'pending_count': 0
                                        }, room=sid)
                            else:
                        ...
                        if target_username in user_data['friends']:
                            ...
                            if save_user_data(user_data) and save_user_data(target_user_data):
                                ...
                                message_type = "success"
                                # Real-time: tell both sides to remove the other from sidebar
    ...
    if request.method == 'POST':
        ...
        if section == 'theme':
            ...
            if new_primary and new_accent:
                # ── FIX: Validate hex colour format ──
                ...
                message = "Both colours are required."
                message_type = "error"
            ...
            if new_email and new_email != user_data.get('email', ''):
                if '@' not in new_email or '.' not in new_email.split('@')[-1]:
                    ...
                    email_exists = any(
                        udata.get('email', '').lower() == new_email
...
def admin_page():
    ...
    if request.method == 'POST':
        ...
        if action in ('delete_user', 'timeout', 'untimeout', 'ban_user'):
            if not target_username or target_username not in all_users_data:
                message = f"User '{target_username}' not found."
                message_type = "error"
            elif target_username == 'admin':
                message = "Cannot perform this action on admin."
                message_type = "error"
            else:
                try:
                    if action == 'delete_user':
                        ...
                        if user_to_ban and user_to_ban.get('email') not in banned_emails:
                            banned_emails.append(user_to_ban['email'])
                            if save_banned_emails(banned_emails):
                                message = f"User '{target_username}' banned."
                                message_type = "success"
                            else:
                                message = "Failed to ban user."
                                message_type = "error"
                        else:
                            message = f"Email for '{target_username}' already banned or user not found."
                            message_type = "error"
    ...
    for uname, data in all_users_data.items():
        if uname != 'admin':
            users_for_template.append(User(uname, uname, data['email'],
                                           data['password'], data.get('friends'),
                                           data.get('requests'), data.get('blocked'),
                                           data.get('chat_history'), data.get('settings'),
                                           data.get('timeout_until'),
...
def handle_connect():
    if current_user.is_authenticated:
        ...
        if username not in user_sids:
            user_sids[username] = []
        if request.sid not in user_sids[username]:
...
def handle_disconnect():
    if current_user.is_authenticated:
        username = current_user.username
        if username in user_sids:
            user_sids[username] = [s for s in user_sids[username] if s != request.sid]
...
def on_join(data):
    try:
        ...
        if other_user not in user_data.get('friends', []):
            emit('error', {'message': f'You are not friends with {other_user}.'}, room=request.sid)
            return

        if username in target_user_data.get('blocked', []):
            emit('error', {'message': f'You are blocked by {other_user}.'}, room=request.sid)
            return
...
def handle_message(data):
    try:
        ...
        if current_user.is_timed_out:
            emit('error', {'message': f'You are timed out until {current_user.timeout_until.strftime("%Y-%m-%d %H:%M")}.'}, room=request.sid)
            return

        # ── FIX: Reject empty messages ──
        ...
        if not check_rate_limit(username):
            emit('error', {'message': 'You are sending messages too quickly. Please wait a moment.'}, room=request.sid)
            return

        # ── FIX: Verify the user is authorised for the room before broadcasting ──
        if room.startswith('group_'):
            group_chats = load_group_chats()
            ...
            if len(parts) != 2 or username not in parts:
                emit('error', {'message': 'Invalid room.'}, room=request.sid)
                return
            other_user = parts[1] if parts[0] == username else parts[0]
            user_data = load_user_data(username)
            if other_user not in (user_data or {}).get('friends', []):
        ...
        app.logger.info(f'Message from {username} in {room}: {msg[:80]}')
...
def handle_edit_message(data):
    try:
        ...
        if current_user.is_timed_out:
            emit('error', {'message': 'You are timed out. Cannot edit messages.'}, room=request.sid)
            return

        if len(new_text) > MAX_MESSAGE_LENGTH:
            emit('error', {'message': f'Message too long. Maximum {MAX_MESSAGE_LENGTH} characters.'}, room=request.sid)
            return
        ...
        if message_found:
            ...
            emit('error', {'message': 'Message not found or no permission.'}, room=request.sid)
    except Exception as e:
...
def handle_delete_message(data):
    try:
        ...
        if current_user.is_timed_out:
            emit('error', {'message': 'You are timed out. Cannot delete messages.'}, room=request.sid)
            return
...
def handle_user_connected():
    try:
        username = current_user.username
        if current_user.is_timed_out:
            emit('error', {'message': 'You are temporarily timed out.'}, room=request.sid)
            return
...
def handle_webrtc_offer(data):
    try:
        ...
        room = data['room']
        offer = data['offer']
        emit('webrtc_offer', {'offer': offer, 'sender': sender}, room=room, include_self=False)
    except Exception as e:
...
def handle_webrtc_answer(data):
    try:
        ...
        room = data['room']
        answer = data['answer']
        emit('webrtc_answer', {'answer': answer, 'sender': sender}, room=room, include_self=False)
    except Exception as e:
...
def handle_ice_candidate(data):
    try:
        ...
        emit('webrtc_ice_candidate', {'candidate': candidate, 'sender': sender}, room=room, include_self=False)
    except Exception as e:
...
def handle_group_call_reject(data):
    try:
        username = current_user.username
        call_room = data['call_room']
        emit('group_call_rejected', {'username': username, 'call_room': call_room}, room=call_room)
    except Exception as e:
